...
### 4. `/api/firstnames/history` - Get Historical Data for a First Name

Retrieves historical popularity data for a specific first name across different years.

**Query Parameters:**

- `name`: (String, required) The first name to retrieve historical data for. Case-insensitive.
- `gender`: (String, optional) Filter the historical data by gender. Valid values:
- `"male"`: Returns only male name history.
- `"female"`: Returns only female name history.
- If not specified, returns history for the name regardless of gender.
  If an invalid gender is provided, the API returns a 400 error with message: "Invalid gender specified".
...
- 400 Bad Request:
- If no name parameter is provided: `{"errors": [{"message": "Name parameter is required"}]}`
- If an invalid gender is specified: `{"errors": [{"message": "Invalid gender specified"}]}`
- 404 Not Found:
- If the requested name doesn't exist in the database: `{"errors": [{"message": "Name not found"}]}`

**Example Requests:**

- `/api/firstnames/history?name=Michael` (Historical data for the name "Michael")
- `/api/firstnames/history?name=Pat&gender=female` (Historical data for the female name "Pat")
