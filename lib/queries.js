...
var fastSimpleQuery, frequencyQuery, genderQuery, isNumber, isUndefined, knex, limitQuery, raceQuery, yearQuery, _;
...
genderQuery = function(context, req_gender) {
  ...
  if (validGenders.indexOf(req_gender.toLowerCase()) > -1) {
    gender = {
      'male': 'M',
      'female': 'F'
    };
    return context.where({
      gender: gender[req_gender]
    });
  } else {
    return context.where({
      year: 0
