...
var anyFrequency, anyRace, anyRank, async, errorHandler, fastSimpleQuery, frequencyQuery, genderQuery, isNumber, isUndefined, knex, limitQuery, properCase, raceQuery, rankQuery, sanitizeGender, sanitizeYear, startsWithQuery, yearQuery, _;
...
startsWithQuery = function(context, req_letter, errorHandler) {
  ...
  if (req_letter) {
    if (!req_letter.match(/[^a-zA-Z]/)) {
      ...
      return context.where("name", "NOT LIKE", properCase(not_match) + "%");
    } else {
      context.where("name", "LIKE", "%");
      return errorHandler.addError(errorHandler.errorCodes['invalid_startswith']);
    }
  } else {
